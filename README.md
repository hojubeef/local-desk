...
Local Desk의 `Git` 탭에서 저장소 상태 확인, 커밋, 받기, 올리기를 할 수 있습니다.

- `커밋`: 현재 변경사항을 로컬 Git에 저장합니다.
- `받기`: GitHub 변경사항을 `git pull --ff-only`로 가져옵니다.
- `올리기`: 커밋된 내용을 GitHub에 push합니다.

`올리기`는 GitHub로 데이터가 전송되므로 버튼을 누를 때 확인창이 뜹니다.
